...
const App = () => {
  return (
    <>
      ...
      <Layout title="Layout 1 text" urlBg={urlBG1} id="rules">
        <p>
      ...
      <Layout title="Layout 2 text" colorBg="red" id="cards">
        <p>Some text...</p>
        <p>Some text...</p>
        <img className="image-logo" src={logoPng} alt="logo" />
        <div className="flex">
          {POKEMONS.map((item) => (
            <PokemonCard key={item.id} id={item.id} name={item.name} img={item.img} type={item.type} values={item.values} />
          ))}
        </div>
      </Layout>
      <Layout title="Layout 3 text" urlBg={urlBG2} id="about">
        <p>Some text...</p>
        <p>Some text...</p>
        <p>Some text...</p>
      </Layout>
